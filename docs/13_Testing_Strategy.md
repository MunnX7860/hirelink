...
| `tests/screening.test.ts`        | always-on | questionnaire zod schemas (≤20 questions, option caps, id shape), **sanitizer strips classification+rule** from the public projection, answers payload validation (unknown-id drop / missing-required field errors), `ScreeningResultSchema` heal/reject + INSUFFICIENT_EVIDENCE vocabulary, prompt builders (injection + non-negotiables asserted, candidate-label anonymization), session-create pure guards (pool mapping, result grouping/top-N; live 409 concurrent covered DB-gated by S9), migration 0007 SQL sanity                                                                                                                                                                                  |
