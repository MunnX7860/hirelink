...
| **Storage**       | Org-level Drive row wins; falls back to the **job creator's** personal integration; else degraded (03 §6). Org connects use a shared company Google account — the same OAuth flow, `state` carries `org` (`POST /api/integrations/google/start` body `{ org_id }`, admin+ capability `integrations.manage`) |
...
3. First login after activation with ≥1 membership → org chooser surfaces once ("Stay personal" / pick org); choice persists to `users.default_organization_id`. **Implemented in migration `0010_workspace_chooser.sql`**: `users.workspace_onboarded_at` is the dedicated "has chosen" signal (`default_organization_id IS NULL` alone can't distinguish "never chosen" from "explicitly chose personal"), backfilled `now()` only for users who already had an explicit default org; `needsWorkspaceChooser()` (`features/orgs/server.ts`) gates the client `WorkspaceChooserModal` (`features/orgs/workspace-chooser-modal.tsx`), mounted in the authenticated app shell; both choices go through the existing `switchWorkspace()` path, which now also stamps the flag. The standalone runner `scripts/backfill-orgs.mjs` mirrors step 1 for users created mid-cutover (idempotent, `--dry-run` supported).
