...
| Route                                       | Purpose                                                                                                                                                                                   |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /api/integrations/google/start`       | Body `{ org_id?: uuid }` (org context, §4.10 clarification below) — returns Google OAuth URL (Drive scope, offline) with signed `state`                                                   |
...
| `GET /api/users/me/export`       | Downloads a JSON bundle of the caller's PERSONAL-workspace data (docs/02 §9 "Export my data"; org-shared data excluded)                                                                                                                                        |
| `DELETE /api/users/me`           | Deletes the caller's own account (docs/02 §9 "Delete account") — DB rows + stored tokens via cascade, Drive files untouched. 409 `CONFLICT` if the caller still owns a live org or org-scoped jobs/candidates/tags/integrations (must reassign/transfer first) |
...
| `GET /api/cron/org-purge` (Phase 4)   | Vercel cron, daily 04:00 UTC; same bearer-gate as `reconcile` (401/404). Hard-deletes organizations past the 7-day soft-delete grace window after re-homing their `jobs`/`applicants`/`tags`/`integrations` rows to `organization_id = null` (never destroyed, 11 §7). Returns `{ orgs_purged, rows_rehomed }` |
...
| `GET /api/ai/screenings?job_id=`         | Session list for a job (append-only history) `[{ id, pool, max_results, instruction, status, pool_size, processed, failed, engine, created_at, started_at, completed_at, created_by_name, summary_file_id }]` (`summary_file_id` = Drive summary artifact when written, 17 §13)                                                                                                            |
| `GET /api/ai/screenings/:id`             | Session detail + `results` (streamed incrementally while `processing` — progress view shows partial groups; grouped by category, ordered rank; per-row `reasons/evidence/uncertainties`). **Advance-on-view** (17 §9.1): while the session is active, the same request kicks one post-response processing slice via `after()` — the dashboard's polling doubles as the Hobby self-healer   |
| `POST /api/ai/screenings/:id/retry`      | Re-queue `failed` rows individually + revive the terminal session (17 §9.3; batch provenance reset, upgrade re-decides on real pending volume) → 202 `{ requeued, resumed }` when work remains; honest no-op 200 `{ requeued: 0, resumed: 0 }` when nothing is left. 409 while the session is still running                                                                                |
| `POST /api/ai/screenings/:id/cancel`     | Cancel a `queued`/`processing`/`quota_limited` session → 200 (completed rows retained; pending rows stay unscreened and a later retry revives them; a live Gemini Batch is cancelled remotely best-effort — D4). 409 when the session already reached a terminal state                                                                                                                     |
| `GET /api/cron/screening-worker`         | Bearer-gated worker tick (17 §9): scans the active-sessions partial index (≤10), CAS-claims ≤3 claimable sessions and advances each one time-boxed slice within a 45 s budget (interactive ≤8-candidate chunks, or batch poll/drain). 404 without `CRON_SECRET` configured, 401 on wrong bearer                                                                                            |
