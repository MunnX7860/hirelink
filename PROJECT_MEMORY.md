...
- **Phase 5 — Smart Screening (docs/17) · Stage 5.0 docs (2026-08-09) + Stage 5.1 CODE COMPLETE (2026-08-09).** Questionnaire + deterministic engine shipped: `jobs.screening_config` (PRIVATE rules; `sanitizeQuestions()` is the ONLY candidate-facing projection — Q7), pure `features/screening/engine.ts` (safety rail: any clear fail → `does_not_meet_mandatory`; any ambiguous/empty → `review_required`; no mandatory → NULL; 100 % branch-covered), verdicts on `applications.screening_status` (pipeline status stays human-owned), apply multipart `answers` field → 400 `VALIDATION_ERROR` w/ `answers.<id>` field errors, first-write-wins, persistence failures absorbed (D4); questionnaire builder + sync re-run (200 `{recomputed, changed}` — intentionally NOT the §9 worker) + answers/verdict card on application detail; routes `GET/PUT jobs/:id/screening-config`, `POST jobs/:id/screening-recompute`, `GET applications/:id/answers`; **migration 0007** (+ `application_answers`, `applicant_profiles`, `ai_screening_sessions/results` — AI tables inert until 5.2+; works with ZERO AI key). Gates: **281 unit + 53 always-on e2e all green**; `/apply/[slug]` 5.63 kB. Next: 5.2 resume profiles → 5.3 sessions → 5.4 async worker/Batch → 5.5 results/Drive → 5.6 500-candidate scale gate.
